      ...
      {badges.map(b => (
        <div key={b.id} className="bg-surface rounded-xl p-5 shadow-sm space-y-3">
          <div className="flex items-center gap-3">
            ...
            >
              <div className={`w-5 h-5 bg-surface rounded-full shadow transition-transform ${b.is_active ? 'translate-x-4' : ''}`} />
            </div>
          ...
          <button
            ...
            className="text-sm btn-gradient text-gray-900 px-4 py-1.5 rounded-lg hover:brightness-95 disabled:opacity-50 transition-colors"
          >
            {saving === b.id ? '儲存中...' : '儲存'}
          </button>
        </div>
